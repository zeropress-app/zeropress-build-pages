...
async function buildFrontPageData(frontPageConfig, pageInputs, config2) {
  ...
  return {
    ...
    page: {
      ...
      content: html,
      document_type: "html",
      excerpt: extractHtmlExcerpt(html) || config2.site?.description || "",
      status: "published"
    }
  };
}
...
function extractRawAbsolutePath(value) {
  const match2 = /^[A-Za-z][A-Za-z0-9+.-]*:\/\/[^/?#]*(?<path>[^?#]*)/u.exec(value);
  return match2?.groups?.path || "/";
}
...
function extractExcerpt(markdown, title) {
  const paragraphs = markdown.split(/\n{2,}/).map((block2) => block2.trim()).filter(Boolean).filter((block2) => !isMarkdownHeadingBlock(block2)).filter((block2) => !block2.startsWith("```"));
...
function extractHtmlExcerpt(html) {
  const text2 = html.replace(/<script\b[\s\S]*?<\/script>/gi, " ").replace(/<style\b[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#39;/gi, "'").replace(/\s+/g, " ").trim();
  return text2.slice(0, 240);
}
function isMarkdownHeadingBlock(block2) {
  if (block2.startsWith("#")) {
    return true;
  }
...
function splitLinkTarget(target) {
  const match2 = target.match(/^([^?#]*)([?#].*)?$/);
  return {
    pathname: match2?.[1] || target,
    suffix: match2?.[2] || ""
  };
}

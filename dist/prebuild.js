...
function buildSiteData(config, frontPage) {
  const configuredSite = isPlainObject(config.site) ? config.site : normalizeSiteConfig(void 0);
  const site = {
...
async function buildFrontPageData(frontPageConfig, pageInputs, config) {
  ...
  if (frontPageConfig.type === "markdown") {
    ...
    assertUniquePageSlug(pageInputs, matchedPage.route.slug, sourcePath2);
    return {
      frontPage: {
        type: "page",
...
function normalizeSourceFilePath(value, pathLabel) {
  ...
  if (path.isAbsolute(normalizedPath) || normalizedPath.includes("?") || normalizedPath.includes("#") || segments.some((segment) => !segment || segment === "." || segment === "..")) {
    throw new PrebuildConfigError(
...
function parseMarkdownSource(rawMarkdown, sourcePath) {
  try {
    const parsed = (0, import_gray_matter.default)(rawMarkdown);
...
function normalizeFrontMatterRoutePath(value, sourcePath) {
  ...
  if (routePath.startsWith("/") || routePath.endsWith("/") || routePath.includes("\\") || routePath.includes("?") || routePath.includes("#") || segments.some((segment) => !isSafeRoutePathSegment(segment))) {
    throw new PrebuildMarkdownError(
      sourcePath,
...
function buildPageRoute(sourcePath, options2 = {}) {
  const relativePath = path.relative(sourceDir, sourcePath).replace(/\\/g, "/");
  const routePath = buildRoutePath(relativePath, sourcePath, options2);
...
function buildHtmlPageRoute(sourcePath, options2 = {}) {
  const relativePath = path.relative(sourceDir, sourcePath).replace(/\\/g, "/");
...
function buildRoutePath(relativeSourcePath, sourcePath, options2 = {}) {
  if (options2.routePath) {
    if (options2.routePath === "index" && !options2.allowRootIndex) {
      throw new PrebuildMarkdownError(
        sourcePath,
    ...
    return options2.routePath;
  }
  const extensionPattern = options2.extensionPattern || /\.md$/i;
  const withoutExtension = relativeSourcePath.replace(extensionPattern, "").toLowerCase();
  const segments = withoutExtension.split("/").map((segment) => sanitizePathSegment(segment)).filter(Boolean);
  const routePath = segments.join("/");
  if (routePath === "index" && options2.allowRootIndex) {
    return routePath;
  }
...
function extractExcerpt(markdown, title) {
  const paragraphs = markdown.split(/\n{2,}/).map((block) => block.trim()).filter(Boolean).filter((block) => !isMarkdownHeadingBlock(block)).filter((block) => !block.startsWith("```"));
  const first = paragraphs[0] || title;
  return first.replace(/^>\s*/gm, "").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").replace(/[`*_]/g, "").replace(/\s+/g, " ").trim().slice(0, 240);
}
function extractHtmlExcerpt(html) {
  const text = html.replace(/<script\b[\s\S]*?<\/script>/gi, " ").replace(/<style\b[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#39;/gi, "'").replace(/\s+/g, " ").trim();
